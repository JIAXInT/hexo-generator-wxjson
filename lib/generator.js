...
module.exports = function (hexo, site) {
  ...
  let restful = cfg.hasOwnProperty('restful_api') ? cfg.restful_api :
    ...
    posts = site.posts.filter(function (post) {
      ...
      // 排序条件：首先按 sticky 排序（大的在前），然后按 date 排序（新的在前）
      if (b.sticky !== a.sticky) {
        return b.sticky - a.sticky; // sticky 值大的优先
      }
      return b.date - a.date; // sticky 相同的情况下，按日期排序
    }),
    ...
    postMap = function (post) {
      return {
        ...
        sticky: posts_props('sticky', post.sticky) // 添加sticky属性
      };
    },
