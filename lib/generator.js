...
module.exports = function (hexo, site) {
  ...
  let restful = cfg.hasOwnProperty("restful_api")
      ? cfg.restful_api
      : {
          site: true,
          posts_size: 10,
    ...
    posts = site.posts
      .map((post) => {
        // 确保sticky是数字，没有的则为0
        post.sticky = typeof post.sticky === "number" ? post.sticky : 0;
        return post;
      })
      .sort(function (a, b) {
        // 首先比较 sticky 值
        if (b.sticky !== a.sticky) {
          return b.sticky - a.sticky; // 从大到小排序
        }
        // 如果 sticky 相同，则按日期排序
      ...
      return {
        ...
        categories: posts_props("categories", function () {
          return post.categories.map(function (cat) {
            let name = cfg.restful_api.use_category_slug && cat.slug ? cat.slug : cat.name;
            return {
              name: name,
        ...
        tags: posts_props("tags", function () {
          return post.tags.map(function (tag) {
            let name = cfg.restful_api.use_tag_slug && tag.slug ? tag.slug : tag.name;
            return {
              name: name,
        ...
        // 添加sticky属性，如果没有则为0
        sticky: posts_props("sticky", post.sticky || 0),
      };
    },
